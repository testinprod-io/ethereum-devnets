...
locals {
  digitalocean_vm_groups = flatten([
    for group_index, vm_group in local.vm_groups :
    [
      for i in range(0, vm_group.count) : {
        ...
        vms = {
          "${i + 1}" = {
            ...
            region = element(var.digitalocean_regions, (i == 3 ? 6 : i % length(var.digitalocean_regions)))
...
resource "cloudflare_record" "xatu-server" {
  ...
  type    = "CNAME"
  value   = "xatu.${var.ethereum_network}.yolkie.xyz"
  proxied = false
  ttl     = 120
}
...
resource "cloudflare_record" "server_record_ethstats_v4" {
  for_each = {
    for vm in local.digitalocean_vms : "${vm.id}" => vm if contains(vm.tags, "group_name:ethstats")
  }
  zone_id = data.cloudflare_zone.default.id
  name    = "ethstats.${var.ethereum_network}"
  type    = "A"
  value   = digitalocean_droplet.main[each.value.id].ipv4_address
  proxied = false
  ttl     = 120
...
resource "cloudflare_record" "ethstats-server" {
  zone_id = data.cloudflare_zone.default.id
  name    = "server.ethstats.${var.ethereum_network}"
  type    = "CNAME"
  value   = "ethstats.${var.ethereum_network}.yolkie.xyz"
  proxied = false
  ttl     = 120
}

resource "cloudflare_record" "ethstats-wildcard" {
  zone_id = data.cloudflare_zone.default.id
  name    = "*.ethstats.${var.ethereum_network}"
  type    = "CNAME"
  value   = "ethstats.${var.ethereum_network}.yolkie.xyz"
...
resource "local_file" "ansible_inventory" {
  ...
  filename = "../../ansible/inventories/pectra-devnet-ssl-2/inventory.ini"
}
